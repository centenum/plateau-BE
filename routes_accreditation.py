...
def auto_accreditation_step1():
    ...
    try:
        ...
        print(verification_data, verification_data.get('VIN', '').replace(' ', ''))
        if 'VIN' not in verification_data:
            return jsonify({'message': 'Invalid voter\'s card'}), 400
        
        voter = voter_collection.find_one({'VIN': verification_data.get('VIN', '').replace(' ', '')})
        if not voter:
    ...
    for unit in polling_units:
        ...
        if not unit.get('lga'):
            ...
            if len(wards) > 0:
                ...
                random_lga = random.choice(wards_data).get("LGA NAME")
                unit['lga'] = random_lga
                db.polling_units.update_one({"_id": unit.get('_id')}, {"$set": {
                    "lga": random_lga,
                }})
